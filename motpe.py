...
def nondominated_sort(points):
    ...
    while c > 0:
        ...
        # print("swapaxes:", np.swapaxes(extended, 0, 1).shape, np.swapaxes(extended, 0, 1)[0])
        dominance = np.sum(
            np.all(extended < np.swapaxes(extended, 0, 1), axis=2), axis=1)
        # print("dominance:", dominance)
        
        # where index is 0, it means that the point is not dominated by any other point
        # once we sorted these points, blow it up so that we know it is already ranked
...
class TPESampler:
    def __init__(self,
        ...
        self.hp = hp  # Set the hyperparameters
        self._observations = observations  # Set the observations
        self._random_state = random_state  # Set the random seed
        self.n_ei_candidates = n_ei_candidates  # Set the number of candidates for EI maximization
        self.gamma_func = gamma_func  # Set the function to compute the gamma parameter
        self.weights_func = weights_func  # Set the function to compute the weights
        self.opt = self.sample  # Set the next point to sample to be the sample function
        self.rule = rule  # Set the rule to use
        if split_cache:  # If split cache is provided
            self.split_cache = split_cache  # Set the split cache
        else:
            self.split_cache = dict()  # Otherwise, create an empty dict

    def sample(self):
        # Load the observed hyperparameter values and corresponding function values.
        hp_values, ys = self._load_hp_values()

        # Split the observations into lower and upper sets of hyperparameter values
